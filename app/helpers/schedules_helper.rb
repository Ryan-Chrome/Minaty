module SchedulesHelper
  ...
  def total_schedule_circle(user, objs)
    ...
    names = objs.group_by { |x| x.name }.map(&:first)
    names.each do |name|
      target_objs = objs.select { |x| x.name == name }
      work_time = 0
      target_objs.each do |t|
        start_time = Time.parse(t.start_at)
        end_time = Time.parse(t.finish_at)
        work = ((end_time - start_time) / 3600).floor(1)
        work_time += work
      end
      schedule_list << [name, work_time]
    end
    schedule_list.sort! { |a, b| b[1] <=> a[1] }
    return schedule_list[0..9]
  end

  def month_schedule_circle(user, objs)
    schedule_list = Array.new
    names = objs.group_by { |x| x.name }.map(&:first)
    names.each do |name|
      target_objs = objs.select { |x| x.name == name }
  ...
  def week_schedule_circle(user, objs)
    schedule_list = Array.new
    names = objs.group_by { |x| x.name }.map(&:first)
    names.each do |name|
      target_objs = objs.select { |x| x.name == name }
  ...
  def create_schedule_parsent(array, work_time)
    ...
    total_parsent = 0
    array.each do |obj|
      parsents << [obj[0], (((obj[1] + total_parsent) * 100 / work_time)).floor.to_s + "%"]
      total_parsent += obj[1]
    end
    return parsents
  end
