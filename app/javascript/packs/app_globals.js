$(document).on('turbolinks:load', () => {
  ...
  var attendances_path = document.getElementById("attendance-container");
  ...
  if(notice_close) {
    ...
    notice_close.onclick = (e) => {
      notice_content.style.display = "none";
  ...
  if (home_index.length) {
    ...
    if (offset_height == scroll_height && total_page > 1) {
      var home_ajax_message_date = document.getElementsByClassName("message-date")[0].id;
      var message_content_scroll = general_chat_container.scrollHeight;
      $.ajax({
        url: `/general_messages/home_chat_ajax?page=${page_number}&message_date=${home_ajax_message_date}`,
      }).done(function (data) {
    ...
    // ルームの状態確認
    var room_status = document.getElementById("video-config-container");
    if(room_status.dataset.room_status === "on") {
      // サイドバー展開
        ...
        if(e.target.classList.contains("active")){
          ...
          if(activeElm){
            ...
            if(target_opener == message_container){
              ...
              var chat_notice = document.getElementById("room-chat-notice");
              if(chat_notice.style.display == "inline-block"){
  ...
  } else if (attendances_path) {
    // ダウンロードコンテナ関連
    var download_header = document.getElementById("download-header");
    var download_form = document.getElementById("download-form");
    var download_start_date = document.getElementById("start_date");
    var download_finish_date = document.getElementById("finish_date");
    var download_btn = document.getElementById("download-btn");
    download_header.onclick = () => {
      if(download_form.classList.contains("active")){
        download_form.classList.remove("active");
      } else {
        download_form.classList.add("active");
      }
    }
    download_btn.onclick = () => {
      var start_date = Date.parse(download_start_date.value);
      var finish_date = Date.parse(download_finish_date.value);
      if(!download_start_date.value && !download_finish_date.value) {
        var start_date = Date.parse(download_start_date.value);
        var finish_date = Date.parse(download_finish_date.value);
        alert("フォームに不備があります");
      }
